...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="min-h-screen bg-dark-200 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-semibold text-slate-400">Verifying secure admin credentials...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-dark-200 text-slate-100 flex flex-col md:flex-row">
      {/* Mobile Topbar */}
      <div className="md:hidden flex items-center justify-between p-4 bg-dark-100 border-b border-slate-800">
        <Link href="/admin" className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-amber-500 to-rose-600 flex items-center justify-center font-black text-white text-sm">
            H
          </div>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:sticky top-0 inset-y-0 left-0 z-40 w-64 bg-dark-100 border-r border-slate-800 p-6 flex flex-col justify-between transition-transform duration-300 ${
      ...
      >
        <div className="space-y-8">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 via-rose-500 to-brand-600 flex items-center justify-center shadow-lg">
        ...
        <div className="space-y-3 pt-6 border-t border-slate-800/80">
          ...
          <button
            ...
            disabled={isSeeding}
            className="w-full flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700/80 text-xs font-semibold transition-colors"
          >
      ...
      {/* Main Content Area */}
      <div className="flex-1 min-w-0 p-4 sm:p-6 lg:p-10 max-w-7xl">
        {children}
      </div>
    </div>
  );
